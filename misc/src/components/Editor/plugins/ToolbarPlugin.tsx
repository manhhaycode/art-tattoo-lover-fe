...
function positionEditorElement(editor: HTMLElement, rect: RectType | null) {
    if (rect === null) {
...
export function FillURL() {
    const srcfile = prompt('Nhập đường dẫn URL của ảnh:', '');
    const parttern = /^https:\/\/storage\.googleapis\.com\/arttattoolover-adf51\.appspot\.com\/media\/.*/;
    if (srcfile && !parttern.test(srcfile)) {
...
//     options,
//     value,
// }: HTMLAttributes<HTMLSelectElement> & OptionHTMLAttributes<HTMLOptionElement> & { options: Array<string> }) {
//     return (
    ...
    useEffect(() => {
        if (text) {
            editor.update(() => {
                ...
                const dom = parser.parseFromString(text as string, 'text/html');
                $getRoot().select();
    ...
    const save = useCallback(async () => {
        if (saveEditor) {
            editor.getEditorState().read(async () => {
                await saveEditor($generateHtmlFromNodes(editor, null));
            });
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []);
    ...
    return (
        editable && (
            <div className="toolbar" ref={toolbarRef}>
                ...
                <button onClick={save} className={'toolbar-item spaced'}>
