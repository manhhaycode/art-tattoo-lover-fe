...
import { useFilterFormStore, useModalStore } from '@/store/componentStore';

export default function FilterMobile({ isLoading }: { isLoading?: boolean }) {
    const [value, setValue] = useState<string[]>([]);
    const [searchKeyword, setSearchKeyword] = useState('');
    const { filterData, setFilterData, setIsQuery, isQuery } = useFilterFormStore();
    const { setFilterMobileModal } = useModalStore();
    ...
    return (
        <div className="flex flex-col overflow-auto h-full justify-center w-[calc(100vw-36px)] px-4">
            ...
            >
                ...
                <div className="flex flex-col gap-y-3">
                    <label className="text-placeholder-gray font-medium text-lg">Lọc theo đánh giá</label>
                    <Checkbox.Group
                        value={value}
                        onChange={(values) => {
                            setValue(values);
